...
| Python packages | `tools/requirements.txt`: pydantic 2, jinja2, pillow, lz4, rich, requests, pyelftools | `pydantic` and `rich` are not in EdgeTX's documented list but the hardware-definition generator imports them. `pyelftools` is what `elf2uf2.py` imports; it was missing from the list until 2026-09-18, so the firmware linked and then the build died before writing `firmware.uf2`. |
