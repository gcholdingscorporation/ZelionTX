...
A transmitter firmware for RC helicopters that does three things well, stick
translation, ExpressLRS link control and Rotorflight telemetry, and deliberately does
nothing else.

It is built on the concept that the flight controller (Rotorflight) owns rates, expo,
mixing, governor, rescue and arming logic, so the transmitter only needs to read the
pilot's hands accurately, keep the ELRS link fed, and show and speak what the aircraft
reports through the ZelionDash dashboard that already exists as an EdgeTX widget.
